...
class CableCalculatorApp:
    ...
    def calculate(self):
        try:
            ...
            # 参数验证
            if any(v <= 0 for v in [d1, d2, l2, D]):
                raise ValueError("d1, d2, l2, D 必须为正数")
            if t < 0:
                raise ValueError("装盘余量不能为负数")
            if d1 <= d2:
            ...
            avg_diameter = d2 + 0.1 * D + 0.9 * D * P_floor
            L = (math.pi * P_floor * n_floor * avg_diameter) / 1000

            # 显示结果
            self.results["P"]["widget"].config(text=f"{P:.1f} 层")
            self.results["n"]["widget"].config(text=f"{n:.1f} 圈")
